...
export class SalonController {
  ...
  @ApiQuery({ name: 'search', required: false, type: String, example: 'Natural Hair' })
  @ApiQuery({ name: 'location', required: false, type: String, example: 'Sydney' })
  @ApiQuery({ name: 'minRating', required: false, type: Number, example: 4.5 })
  @ApiQuery({ name: 'services', required: false, type: [String], example: ['Haircut', 'Coloring'] })
  @ApiQuery({ name: 'sortBy', required: false, enum: ['bestMatch', 'topRated', 'distance'], example: 'bestMatch' })
  ...
  async findAll(@Query() query: any) {
    const options = {
      ...
      services: Array.isArray(query.services) ? query.services : query.services ? [query.services] : [],
